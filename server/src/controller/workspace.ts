...
export const attachFile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const uploadPromises = (req.files as Express.Multer.File[]).map(file => uploadFile(file));
...
console.log('reached')
  ...
  try {
    ...
    return res.status(200).json(ApiSuccess("Workspace created successfully", data));
  } catch (error) {
    console.log(error)
    return res.status(500).json(ApiError("Internal server error", 500, error));
  }
